package com.github.fantyflow.eventbusplugin
...
private class CellRenderer : TableCellRenderer {
    ...
    override fun getTableCellRendererComponent(
        ...
        val bg = if (isSelected) UIUtil.getListSelectionBackground(true) else list.background
        val fg = if (isSelected) UIUtil.getListSelectionForeground(true) else list.foreground
        panel.background = bg
        panel.foreground = fg

        val textChunks = SimpleColoredComponent()
        val offset = 2
        textChunks.ipad = JBUI.insets(offset, offset, offset, 0)
        ...
        when (column) {
            0 -> {
                ...
                textChunks.ipad = JBUI.insets(offset, 0, offset, 15)
